...
module.exports = {
  ...
  async getSinglePost(req, res) {
    try {
      const post = await Post.findOne({ _id: req.params.courseId })
        .select('-__v');
  ...
  async deletePost(req, res) {
    try {
      const post = await Post.findOneAndDelete({ _id: req.params.courseId });

      if (!post) {
        return res.status(404).json({ message: 'No post with that ID' });
      }

      await blogger.deleteMany({ _id: { $in: post.bloggers } });
